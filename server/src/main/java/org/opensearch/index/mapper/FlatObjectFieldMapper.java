...
public final class FlatObjectFieldMapper extends DynamicKeyFieldMapper {
    ...
    public static class Builder extends FieldMapper.Builder<Builder> {
        ...
        public FlatObjectFieldMapper build(BuilderContext context) {
            ...
            return new FlatObjectFieldMapper(name, Defaults.FIELD_TYPE, fft, CopyTo.empty(), this);
        }
    }
    ...
    public static final class FlatObjectFieldType extends StringFieldType {
        ...
        public Query termQuery(Object value, @Nullable QueryShardContext context) {
            failIfNotIndexed();
            Query query = new TermQuery(new Term(getSearchField(), indexedValueForSearch(rewriteSearchValue(value))));
            if (boost() != 1f) {
        ...
        public Query termsQuery(List<?> values, QueryShardContext context) {
            ...
            return new TermInSetQuery(getSearchField(), bytesRefs);
        }
         ...
         * then rewrite the input as the format "dotpath=value",
        ...
        public Query prefixQuery(String value, MultiTermQuery.RewriteMethod method, boolean caseInsensitive, QueryShardContext context) {
            ...
            final String rewriteValue = rewriteSearchValue(value);
            if (caseInsensitive) {
            ...
            return new PrefixQuery(new Term(getSearchField(), indexedValueForSearch(rewriteValue)), method);
        }

        @Override
        public Query rangeQuery(Object lowerTerm, Object upperTerm, boolean includeLower, boolean includeUpper, QueryShardContext context) {
            String directSubfield = getSearchField();
            String rewriteUpperTerm = rewriteSearchValue(upperTerm);
            String rewriteLowerTerm = rewriteSearchValue(lowerTerm);
            if (context.allowExpensiveQueries() == false) {
                throw new OpenSearchException(
        ...
        public Query existsQuery(QueryShardContext context) {
            ...
            return new TermQuery(new Term(rootFieldTypeName, indexedValueForSearch(name())));
        }
    ...
    FlatObjectFieldMapper(String simpleName, FieldType fieldType, FlatObjectFieldType mappedFieldType, CopyTo copyTo, Builder builder) {
    ...
    private void parseToken(XContentParser parser, ParseContext context, Deque<String> path, HashSet<String> pathParts) throws IOException {
        if (parser.currentToken() == XContentParser.Token.FIELD_NAME) {
            ...
            if (fieldType().isSearchable() || fieldType().isStored()) {
                context.doc().add(new Field(valueFieldName, new BytesRef(value), fieldType));
                context.doc().add(new Field(valueAndPathFieldName, new BytesRef(valueAndPath), fieldType));
            }
            if (fieldType().hasDocValues()) {
